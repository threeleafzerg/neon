...
def _Net_init_wrapper(self, model_file, pretrained_file, phase):
    """
    Init_Wrapper for Net
    """
    print "Enter _Net_init_wrapper"
    self.init_impl(model_file, pretrained_file, phase)
    # convert caffe model file into neon graph
    convert = Decaffeinate(model_file, pretrained_file)
    pdict = convert.convert_to_neon()
    # load neon graph into neon model
    default_be = gen_backend(backend='cpu', batch_size=1)
    self.mlp = Model(pdict)

def _Net_forward_wrapper(self, blobs=None, start=None, end=None, **kwargs):
    """ Input data conversion
    """
    input_datas = kwargs['data']
    print "input_datas:", input_datas.shape
    new_data = np.zeros((self.mlp.be.bsz,
                         input_datas.shape[1]*input_datas.shape[2]*input_datas.shape[3]),
                         dtype=np.float32)
    for idx in range(0, len(input_datas)):
        # transpose = input_datas[idx].transpose([1, 2, 0])
        new_data[idx] = input_datas[idx].reshape(1, input_datas.shape[1]*input_datas.shape[2]*input_datas.shape[3])
    """ Prepare neon data set
    """
    inference_set = ArrayIterator(new_data, None, nclass = 10,
                                  lshape=(input_datas.shape[1],input_datas.shape[2],input_datas.shape[3]))
    """ Neon Forward/get_outputs
    """
    neon_outputs = self.mlp.get_outputs(inference_set)
    """ Convert Neon outputs into Caffe Format
    """
    return {out: neon_outputs for out in self.outputs}
...
def _Net_forward(self, blobs=None, start=None, end=None, **kwargs):
    ...
    print "kwargs['data'].shape", kwargs['data'].shape
    if kwargs:
    ...
    print "Enter __Net_forward"
    print outputs
    print "self.blobs['prob'].data.shape:", self.blobs['prob'].data.shape
    print {out: self.blobs[out].data for out in outputs}
    print "Leave __Net_forward"
...
Net.__init__ = _Net_init_wrapper
Net.blobs = _Net_blobs
Net.blob_loss_weights = _Net_blob_loss_weights
Net.params = _Net_params
Net.forward = _Net_forward_wrapper
